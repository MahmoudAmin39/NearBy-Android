...
class RoomAsyncTask(private val callback: VenuesWithPhotosCallback) : AsyncTask<List<Venue>, Void, List<Venue>>() {

    override fun doInBackground(vararg venues: List<Venue>): List<Venue> {
        Log.d("Mahmoud", "Executing Async task")
        val venuesWithPhotos = ArrayList<Venue>()
        venues.first().map { venue ->
            venue.photoUrl = RoomClient.getInstance().databaseInstance?.photoUrlDao()?.getPhotoUrl(venueId = venue.id!!)
...
interface VenuesWithPhotosCallback { fun onVenuesWithPhotosReady(venues: List<Venue>) }